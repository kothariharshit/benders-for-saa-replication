...
class Benders(cflp_benders_utils.CFLPinst):

    def __init__(self, algorithm_details, instance, filename):
        """Intializes and creates the instance data"""

        super().__init__(instance)
        self.load_instance(filename)
        # self.createNewInst(*instance[:3])
        # self.createInst(*instance[:3], 'inst.json')
    ...
    def benders(self, saa_number):
        ...
        def benders_callback(model, where):
            ...
            if where == GRB.Callback.MIPSOL:
                ...
                x_order = tuple(int(self.xvals[ware]) for ware in self.Warehouse)
        ...
        x_order = tuple(self.xvals[ware] for ware in self.Warehouse)
    ...
    def solve_lp_relaxation(self, first, problem_start_time=None):
        ...
        while True:
            ...
            self.solupdate(problem="LP")
            x_order = np.array([self.xvals[ware] for ware in self.Warehouse])
    ...
    def add_cuts_to_master(self, initialize_dict, check_lp_cuts=True):
        ...
        for s in self.scenario:
            for duals in set(initialize_dict[s]):
                ...
                cons = np.dot(self.PI[duals], self.Demand_array[:, s].T)
    ...
    def tech_two_lp_duals(
        ...
        while True:
            ...
            V_sel = {
                solnum: (self.probab * np.sum(vals[solnum]) + ctx_vec[solnum])
                for solnum in V_sel.keys()
            }
    ...
    def active_checker(self):
        ...
        dualctx = np.array(
            [self.capacity[ware] * self.xvals[ware] for ware in self.Warehouse]
        )
        s1 = np.matmul(self.H, dualctx)
        s1 = s1.reshape(-1, 1)
    ...
    def cache_dsp(self, dual_list, lazy=True):
        ...
        sp_optimal, indices = cflp_benders_utils.find_largest_index_numba(
    ...
    def dsp_cuts(self, dual_violations, best_duals, model, scenario_list, lazy=False):
        ...
        s1_vec = [self.capacity[ware] * self.xvals[ware] for ware in self.Warehouse]
                    ...
                    model.cbLazy(
                        (
                            gp.quicksum(
                                self.capacity[i]
                                * self.x[i]
                                * self.H[best_duals[scen], idx]
                                for idx, i in enumerate(self.Warehouse)
                            )
                            + gp.quicksum(
                                self.Demand[scenario, j]
                                * self.PI[best_duals[scen], idx]
    ...
    def technique_one(self, first_solution_list, dual_list):
        ...
        for sol in first_solution_list:
            ...
            s1 = np.matmul(self.H, dualctx)
            s1 = np.squeeze(s1)
            _, indices = cflp_benders_utils.find_largest_index_numba(
                s1[dual_list], self.dual_obj_random[np.ix_(dual_list, self.scenario)]
            )
            duals = [dual_list[id] for id in indices]
...
if __name__ == "__main__":
    ...
    if solve_ip:
        fname = f"detailed-results/cflp/IP/multi_{data}_{match}.op"
    else:
        fname = f"detailed-results/cflp/LP/multi_{data}_{match}.op"
    print("Algo: Multi-Cut", match)

    instance_name = "instances-cflp/" + "_".join(args.data[:3])
    ...
    test = args.data
    np.random.seed(3)
    bend = Benders(args.algorithm_details, test_fl[:4], instance_name)
    ...
    for saa_number in saa_iterations_to_run:
        ...
        # bend.sce_ge_normal(0.1, n_scen, save_filename=filename)
    ...
    df.to_csv(
        "results_multi_cflp.csv",
        index=False,
        quoting=3,
        sep=",",
        escapechar=",",
        mode="a",
        header=not os.path.exists("results_multi_cflp.csv"),
    )
