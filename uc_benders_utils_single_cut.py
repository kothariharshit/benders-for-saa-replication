...
class UCinst:
    ...
    def build_master(self, relaxation=False):
        """
        Build the master problem for Benders decomposition - SINGLE CUT VERSION.

        Master problem contains:
        - Binary commitment variables x[g,t]
        - Minimum up/down time constraints
        - Single second-stage approximation variable z (aggregated across all scenarios)
        ...
        """
        master = gp.Model("UC_Master_SingleCut")
        ...
        # SINGLE CUT: One aggregated second-stage variable instead of z[scenario]
        # The objective coefficient is 1.0 since probability weighting is done in the cuts
        self.z = master.addVar(
            lb=0.0,
            obj=1.0,
        ...
        # Initialize cut tracking for single-cut version
        # cut_history will store lists of dual IDs (one per scenario) for each aggregated cut
        self.cut_history = []
    ...
    def extract_solution_values(self, problem="IP"):
        ...
        # Extract second-stage approximation value - SINGLE CUT VERSION
        # For single-cut, z is a scalar variable, not a dictionary
    ...
    def add_single_cut(self, model, lazy=False, first=False):
        """
        Generate a SINGLE aggregated Benders optimality cut across all scenarios.

        This method solves subproblems for all scenarios and aggregates them into
        one cut with probability weighting, following the single-cut Benders formulation.
        ...
        Returns:
            tuple: (num_cuts_added, aggregated_z_value, total_work_units)
        """
        num_cuts_added = 0
        total_work_units = 0
        ...
        # Initialize aggregated cut expression and tracking
        aggregated_cut_expr = gp.LinExpr()
        aggregated_z_value = 0.0
        cut_dual_ids = []  # Track which dual solution is used for each scenario

        # Loop over all scenarios to build aggregated cut
        for scenario in self.scenario:
            # Update scenario-specific data
            self.update_scenario(self.subproblem, scenario)
            self.subproblem.setParam("OutputFlag", False)
            ...
            subproblem_objective = self.subproblem.ObjVal
            self.scenario_upper_bounds[scenario] = subproblem_objective
            aggregated_z_value += self.probability[scenario] * subproblem_objective
            ...
            # Check if this dual solution is new
            if self.dual_storage.get_index_by_hash(dual_hash) is None:
                # Store new dual solution
                gen_min_ordered = [
                    gen_min_duals[g, t]
            ...
            aggregated_cut_expr += self.probability[scenario] * (
                gp.quicksum(
                    (
                        self.min_power[g] * gen_min_duals[g, t]
                        + self.max_power[g] * gen_max_duals[g, t]
        ...
        # Check if the aggregated cut should be added
        # Compare aggregated_z_value against the current second_stage_values (scalar z)
        if aggregated_z_value - self.second_stage_values > max(
            ...
            if lazy:
                model.cbLazy(self.z >= aggregated_cut_expr)
            else:
                model.addConstr(
                    self.z >= aggregated_cut_expr,
                    name=f"single_benders_cut_{len(self.cut_history)}",
                )

            # Track which dual solutions contributed to this aggregated cut
            self.cut_history.append(cut_dual_ids.copy())

        return num_cuts_added, aggregated_z_value, total_work_units
